...
// src/lib/click-integrity.ts's sibling src/lib/click-token.ts for the full reasoning.
// Deliberately NOT done at render time: this site's SSR creator-listing pages are ISR-cached
// (5min-1h, see revalidate in page.tsx / categories / [locationSlug]) and /api/search is
// CDN-cached too, so anything baked into either would be shared identically by every visitor
// hitting that cache. Running from a plain HTTP client (curl/requests/aiohttp/Go/Java — exactly
// what shows up in this network's real bot traffic) simply never happens, since none of them
// execute page JavaScript at all.
//
